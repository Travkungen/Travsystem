"""Phoenix 15 generic automatic V64 runner."""
...
def fetch_v64(auto_dir: str):
    auto = Path(auto_dir)
    loader = _load(str(auto / "phoenix15_atg_loader_v1.py"), "phoenix15_v64_loader").PhoenixATGLoader()
    adapter = _load(str(auto / "phoenix15_atg_extended_adapter_v2.py"), "phoenix15_v64_adapter").PhoenixATGExtendedAdapter()
    ...
    # ATG exposes the V64 game entries with a V64_ prefix.
    # The Extended Adapter expects the underlying race id without V64_.
    first = str(games[0].get("id", ""))
    if not first.startswith("V64_"):
        raise RuntimeError(f"Oväntat V64-id: {first}")

    first_race = first[4:]
    parts = first_race.rsplit("_", 1)
    if len(parts) != 2 or not parts[1].isdigit():
        raise RuntimeError(f"Kunde inte tolka V64-id: {first_race}")

    prefix, no = parts[0], int(parts[1])
    race_ids = [f"{prefix}_{no+i}" for i in range(6)]
    ...
    for v64_no, race_id in enumerate(race_ids, 1):
        ...
        for start in raw.get("starts", []):
            ...
            rows.append({
                "date": str(today_data.get("date", race_id[:10])),
                "game": GAME,
                "v64": v64_no,
                "race_id": race_id,
    ...
    if df.empty:
        raise RuntimeError("V64 hittades men inga starter hämtades")
    df["winner"] = df["placement"].eq(1)
    return today_data, df


def archive_v64(df: pd.DataFrame, results_dir: str):
