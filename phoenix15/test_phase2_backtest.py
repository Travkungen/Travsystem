...
def test_rule2_uses_market_rank():
    p=PhoenixProtectionLayerV1()
    top=make_ranked(); system=make_system([1,2,3,5,7])
    ...
    assert int(out.in_system.sum())==5
...
def test_multiple_protected_candidates_preserve_size():
    p=PhoenixProtectionLayerV1()
    top=make_ranked(); system=make_system([1,2,3,6,7])
    market=pd.DataFrame([{"start_number":i,"odds_rank":1 if i==4 else 9} for i in range(1,8)])
